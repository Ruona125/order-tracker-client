...
interface Data {
  ...
  Details: string;
  expenses_id: string;
  // costOfOrder: number; // Added property for cost of expenses
}

const columns: readonly Column[] = [
  { id: "ref_no", label: "ref_no", minWidth: 100 },
...
const ExpensesComponent: React.FC = () => {
  ...
  const [expensesToDelete, setExpensesToDelete] = useState<Data | null>(null);

  const handleOpen = (expenses: Data) => {
    setExpensesToDelete(expenses);
    setOpen(true);
  };
    ...
    axios
      ...
      .then(() => {
        setExpenses((prevExpenses) =>
          prevExpenses.filter((expenses) => expenses.expenses_id !== expensesId)
        );
              ...
              {Expenses.filter((Expenses) =>
                ...
                .map((expenses) => (
                  <TableRow
                    ...
                    role="checkbox"
                    tabIndex={-1}
                    key={expenses.expenses_id}
                  >
                    {columns.map((column) => {
                      const value = (expenses as any)[
                      ...
                      if (column.id === "Modify" && roles === "admin") {
                        return (
                          <TableCell key={column.id} align="center">
                            <Link
                              to={`/modify/expenses/${expenses.expenses_id}`}
                            >
                        ...
                        return (
                          <TableCell key={column.id} align="center">
                            <Button
                              variant="contained"
                              color="primary"
                              onClick={() => handleOpen(expenses)} // Call handleDelete on click
                              disabled={deleted}
                            >
                              Delete
                            </Button>
                              ...
                              <Fade in={open}>
                                <Box sx={style}>
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      if (expensesToDelete) {
                                        handleDelete(expenses.expenses_id);
                                      }
                                    }}
                                    color="primary"
                                  >
